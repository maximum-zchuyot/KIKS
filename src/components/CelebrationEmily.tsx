...
export function CelebrationEmily({
  ...
  useEffect(() => {
    ...
    const hearts: Heart[] = Array.from({ length: HEART_COUNT }, (_, i) => {
      ...
      return {
        start: t0 + (i / HEART_COUNT) * FEEDING_MS * 0.85,
        startX,
        startY,
        endX: ex,
        endY: ey,
    ...
    const tick = (now: number) => {
      ...
      for (const heart of hearts) {
        const elapsed = now - heart.start
        if (elapsed < 0) continue
        const t = Math.min(1, elapsed / HEART_FLIGHT_MS)
        if (t >= 1) continue
        liveHearts += 1
        const mt = 1 - t
        const x =
          mt * mt * heart.startX +
          2 * mt * t * heart.controlX +
          t * t * heart.endX
        const y =
          mt * mt * heart.startY +
          2 * mt * t * heart.controlY +
          t * t * heart.endY
        ctx.save()
        ctx.translate(x, y)
        ctx.rotate(heart.spin * t)
